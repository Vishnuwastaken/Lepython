...
# this class displays a window to run a quiz
class Quiz:
    def __init__(self, root, username):
        ...
        self.currentWindow.title("Quiz")
        self.currentWindow.geometry('1000x500')

        # this is for counting the number of attempts
        self.count = 0

        # gives them instructions
        self.lbl_instructions = Label(self.currentWindow, text=f"Hi {self.username.title()}, you will be given a random country, and you have to guess its capital. There will be 3 attempts for you to guess it right. GOOD LUCK!!")
        self.lbl_instructions.pack()

        # they can start quiz/ guess a capital
        self.btn_startquiz = Button(self.currentWindow, text="Guess a capital", command=self.create_question)
        ...
        self.btn_submit = Button(self.currentWindow, text="Submit your answer", command=self.answer)
    ...
    def answer(self):
        ...
        if self.count > 0:
            ...
            if cap == self.capital.lower():
                self.lbl_output.config(text="Congrats! You entered the right capital", fg="green")
                ans = True
